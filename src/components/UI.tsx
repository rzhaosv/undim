...
export function Tag({ text, tone, color }: { text: string; tone?: 'red' | 'gold' | 'plain'; color?: string }) {
  const bg = color ? `${color}26` : tone === 'red' ? colors.emberSoft : tone === 'gold' ? colors.accentSoft : colors.cardAlt;
  const fg = color ? color : tone === 'red' ? colors.ember : tone === 'gold' ? colors.accentDeep : colors.inkSoft;
  return (
